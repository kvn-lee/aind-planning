...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        set1_add = set(node_a1.action.effect_add)
        set1_rem = set(node_a2.action.effect_rem)

        for node1 in set1_add:
            if node1 in set1_rem:
                return True
        
        for node2 in set1_rem:
            if node2 in set1_add:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Let's use sets! We will combine the sets of the actions from node_a1 and node_a2.
        # If there are similarities, then the length of the combined set is positive.
        set1_add = set(node_a1.action.effect_add)
        set2_neg = set(node_a2.action.precond_neg)
        if len(set1_add.intersection(set2_neg)) > 0:
            return True
        
        set2_add = set(node_a2.action.effect_add)
        set1_neg = set(node_a1.action.precond_neg)
        if len(set2_add.intersection(set1_neg)) > 0:
            return True
        
        set1_rem = set(node_a1.action.effect_rem)
        set2_pos = set(node_a2.action.precond_pos)
        if len(set1_rem.intersection(set2_pos)) > 0:
            return True
        
        set2_rem = set(node_a2.action.effect_rem)
        set1_pos = set(node_a1.action.precond_pos)
        if len(set2_rem.intersection(set1_pos)) > 0:
            return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Loop through all the parent nodes and check to see if mutex
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # 
        return node_s1.is_pos != node_s2.is_pos and not node_s1.__eq__(node_s2)
